...
namespace ObjectCache
{
    public class PaymentCache
    {
        ...
        public double GetPaymentTotalByCustomerId(int customerID, DateTime startDate)
        {
            double total = 0;
            double doubleVar;
            OleDbCommand command = DBFunctions.GetInstance().GetCommand(
                "Select P.Amount From Payments As P, Transactions As T, Customers As C " +
                "Where P.TransactionId=T.ID And T.Customer=C.ID And C.ID=? And T.TransactionTime>=?");
        ...
        public void DeletePayment(int paymentId)
        {
            ...
            {
                throw new Exception("Error While Deleting Payment");
            }
        }
        public void Clear()
        {
