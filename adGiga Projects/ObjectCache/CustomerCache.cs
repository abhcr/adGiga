...
namespace ObjectCache
{
    public class CustomerCache
    {
        ...
        {
            double paymentRecievedBefore = 0.0;
            double saleBefore = 0.0;
            //adding purchase and payment made calculations
            double paymentMadeBefore = 0.0;
            double purchaseBefore = 0.0;
            ...
            OleDbCommand command = DBFunctions.GetInstance().GetCommand(
                ...
                "Where T.Customer=? And S.TransactionId=T.ID And T.TransactionTime<? And T.SaleOrPurchase=1");
            ...
            command = DBFunctions.GetInstance().GetCommand(
                ...
                "Where P.TransactionId=T.ID And T.Customer=? And T.TransactionTime<? And T.SaleOrPurchase=1");
            ...
            if (reader.Read())
            {
                if (double.TryParse(reader[0].ToString(), out doubleVar))
                {
                    paymentRecievedBefore = doubleVar;
                }
            }
            command = DBFunctions.GetInstance().GetCommand(
                "Select Sum(S.SaleQuantity*S.SaleRate) From Sales As S, Transactions As T " +
                "Where T.Customer=? And S.TransactionId=T.ID And T.TransactionTime<? And T.SaleOrPurchase=0");
            command.Parameters.AddWithValue("CustomerId", customerID);
            command.Parameters.Add("TransactionTime", OleDbType.Date).Value = beforeDate.Date;
            reader = DBFunctions.GetInstance().GetReader(command);
            if (reader.Read())
            {
                if (double.TryParse(reader[0].ToString(), out doubleVar))
                {
                    purchaseBefore= doubleVar;
                }
            }
            command = DBFunctions.GetInstance().GetCommand(
                "Select Sum(P.Amount) From Payments As P, Transactions As T " +
                "Where P.TransactionId=T.ID And T.Customer=? And T.TransactionTime<? And T.SaleOrPurchase=0");
            command.Parameters.AddWithValue("ID", customerID);
            command.Parameters.Add("TransactionTime", OleDbType.Date).Value = beforeDate.Date;
            reader = DBFunctions.GetInstance().GetReader(command);
            if (reader.Read())
            {
                if (double.TryParse(reader[0].ToString(), out doubleVar))
                {
                    paymentMadeBefore = doubleVar;
                }
            }
            return saleBefore - paymentRecievedBefore - purchaseBefore + paymentMadeBefore;
        }
